...
def get_arguments():
    ...
    parser.add_argument(
        "--loss", default="cos-ce", choices=["cos-ce", "cos", "acos"], help="loss type"
    )
    ...
    parser.add_argument("--attack_type", default="PGD", choices=["PGD", "none"])
...
def train(epoch, model, optimizer, adversary, awp_adversary, trainloader, device, args):
    ...
    elif args.loss == "cos" or args.loss == "acos":
...
def test(is_val, model, test_text_features, adversary, testloader, device, args):
    ...
    for _ in range(repeats):
        for batch_idx, (X, y) in enumerate(testloader):
            ...
            test_clean_acc.update(clean_acc.item() * 100, batch_size)
            test_robust_loss.update(loss_adv.item(), batch_size)
            test_robust_acc.update(robust_acc.item() * 100, batch_size)
    end = time.time()
...
def get_exp_path(args):
    ...
    if args.suffix is None:
        ...
        while os.path.exists(os.path.join(exp_path, str(run_idx))):
            run_idx += 1
        exp_path = os.path.join(exp_path, str(run_idx - 1))
    else:
...
def main():
    ...
    log_file_path = os.path.join(exp_path, "scan.log")
    ...
    zero_shot_loader = data.DataLoader(
        dataset=testset,
        sampler=NShotTaskSampler(testset.ids, args.n_class, 0, args.n_query),
        batch_size=65536,
        pin_memory=True,
        num_workers=2,
    )
    five_shot_loader = data.DataLoader(
        dataset=testset,
        sampler=NShotTaskSampler(testset.ids, args.n_class, 5, args.n_query),
        batch_size=65536,
        pin_memory=True,
        num_workers=2,
    )
    ...
    if not args.eval:
        for epoch in range(starting_epoch, args.all_epoch):
            checkpoint = utils.load_model(
                os.path.join(exp_path, f"model_{epoch}"), backbone, optimizer
            )
            val_meters = test(
                True,
                test_model,
            ...
            if is_best:
                ...
                with open(os.path.join(exp_path, "model_best_zeroshot.pt"), "wb") as f:
                    torch.save(state, f)

            # log this epoch
            logger.info(f"{epoch=}")
            logger.info(utils.get_summary(val_meters))
            logger.info(ENV)
    default_n_test = args.n_test
    checkpoint = utils.load_model(
        os.path.join(exp_path, "model_best"), backbone, optimizer
    )
    logger.info("Test best 5-shot epoch {} on 0-shot".format(checkpoint["epoch"]))
    args.n_test = 400
    test_meters = test(
        False,
        test_model,
    ...
    checkpoint = utils.load_model(
        os.path.join(exp_path, "model_best_zeroshot"), backbone, optimizer
    )
    logger.info("Re-test best 0-shot epoch {} (on 0-shot)".format(checkpoint["epoch"]))
    args.n_test = default_n_test
    test_meters = test(
        False,
        test_model,
        test_text_features,
    ...
    args.n_support = 5
    logger.info("Test best 0-shot epoch {} on 5-shot".format(checkpoint["epoch"]))
    args.n_test = 400
    test_meters = test(
        False,
        test_model,
